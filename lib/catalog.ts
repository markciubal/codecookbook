...
export const SORTING_ALGORITHMS: SortingEntry[] = [
  ...
  { kind: "sorting", name: "Adaptive Sort",  path: "/sorting/adaptive",  time: "O(n log n)", space: "O(log n)",  stable: false, online: false, blurb: "Reads the data before sorting: counting sort for integer ranges, insertion sort for tiny or nearly-sorted inputs, introsort with heapsort fallback otherwise." },
  { kind: "sorting", name: "Pattern-Defeating Quicksort", path: "/sorting/pdqsort", time: "O(n log n)", space: "O(log n)", stable: false, online: false, blurb: "Introsort hybrid hardened against adversarial input. Median-of-3 / ninther pivot, partial-insertion-sort early-out, equal-elements fast path, heapsort fallback. Default sort in Rust's std." },
  ...
  { kind: "sorting", name: "Power Sort",     path: "/sorting/powersort", time: "O(n log n)", space: "O(n)",      stable: true,  online: false, blurb: "TimSort's runs and merges, but ordered by \"node power\" — each boundary's depth in the balanced merge tree — for a near-optimal merge order. CPython's list.sort() since 3.11." },
