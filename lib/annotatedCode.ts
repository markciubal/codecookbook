...
//
// Boilerplate (stats lines, section dividers, run commands, function names) is
// generated by helpers in lib/code-helpers.ts. New algorithms must use those
// helpers — never hardcode comment syntax or naming conventions directly.

import { fnName, statsLine, sectionDivider, runLine } from "./code-helpers";

export type Language = "typescript" | "javascript" | "python" | "java" | "c" | "cpp" | "rust" | "go" | "r";
...
export const LANGUAGE_META: Record<Language, { label: string; ext: string; runCmd: string; commentChar: string }> = {
  ...
  r:          { label: "R",          ext: "R",    runCmd: "Rscript solution.R",                                   commentChar: "#"  },
};

export const LANGUAGES: Language[] = ["typescript", "javascript", "python", "java", "c", "cpp", "rust", "go", "r"];
...
// Run: go run solution.go`,
  ...
  for (i in seq_len(n - 1)) {
    ...
    # Inner pass: compare every adjacent pair in the
    # unsorted prefix and swap if needed.
...
// Run: go run solution.go`,
    ...
    r: `selection_sort <- function(v) {
  n <- length(v)

  # Each pass extends the sorted prefix by one —
  # we place the minimum of v[i:n] at index i.
  for (i in seq_len(n - 1)) {
    min_idx <- i
...
# Time: O(n²)  Space: O(1)  Stable: FALSE

# ── demo ──
data <- c(64, 25, 12, 22, 11)
cat("Before:", data, "\n")
...
// Run: go run solution.go`,
    ...
    r: `insertion_sort <- function(v) {
  n <- length(v)

  # v[1] is trivially sorted. Start at index 2.
  for (i in 2:n) {

    # Pick the element to insert into the
    # already-sorted prefix v[1..(i-1)].
    key <- v[i]
    j   <- i - 1
...
// Run: go run solution.go`,
    ...
    r: `# Stack implemented with an R list (LIFO — last in, first out).
# R has no mutable classes by default; we use an environment for O(1) mutation.

new_stack <- function() {
  e       <- new.env(parent = emptyenv())
  e$items <- list()   # items stored here

  e$push <- function(x) e$items <- c(e$items, list(x))   # add to top
  e$pop  <- function() {
    if (length(e$items) == 0L) stop("Stack underflow")
    val    <- e$items[[length(e$items)]]
    e$items <- e$items[-length(e$items)]
    val
  }
  e$peek <- function() {
    if (length(e$items) == 0L) stop("Stack is empty")
    e$items[[length(e$items)]]
  }
  e$size     <- function() length(e$items)
  e$is_empty <- function() length(e$items) == 0L
  e
}
# All ops: O(1) amortized   Space: O(n)
...
// Run: go run solution.go`,
    ...
    r: `# Queue implemented with an R environment (FIFO — first in, first out).

new_queue <- function() {
  e       <- new.env(parent = emptyenv())
  e$items <- list()

  e$enqueue  <- function(x) e$items <- c(e$items, list(x))   # add to back
  e$dequeue  <- function() {
    if (length(e$items) == 0L) stop("Queue is empty")
    val     <- e$items[[1L]]
    e$items <- e$items[-1L]
    val
  }
...
// Run: go run solution.go`,
    ...
    r: `# Singly linked list in R using recursive environments.
# Each node is an environment with $val and $next.
...
new_linked_list <- function() {
  e       <- new.env(parent = emptyenv())
  e$head  <- NULL
  e$count <- 0L

  e$prepend <- function(val) {        # O(1) insert at front
    n      <- new_node(val)
    n$next <- e$head
    e$head <- n; e$count <- e$count + 1L
  }
  e$delete_head <- function() {       # O(1) remove from front
    if (is.null(e$head)) stop("List is empty")
    e$head  <- e$head$next; e$count <- e$count - 1L
  }
  e$print_list <- function() {        # O(n) traverse and print
    node  <- e$head
    parts <- character(0)
    while (!is.null(node)) { parts <- c(parts, as.character(node$val)); node <- node$next }
    cat(paste(parts, collapse = " -> "), "-> null\n")
  }
  e$size <- function() e$count
  e
}

# ── demo ──
lst <- new_linked_list()
lst$prepend(30); lst$prepend(20); lst$prepend(10); lst$prepend(5)
lst$print_list()     # 5 -> 10 -> 20 -> 30 -> null
lst$delete_head()
lst$print_list()     # 10 -> 20 -> 30 -> null
# Run: Rscript solution.R`,
  },
...
`,
    ...
    r: `# Helper: merge two sorted vectors into one sorted vector
merge_sorted <- function(left, right) {
  result <- integer(length(left) + length(right))
  i <- 1L; j <- 1L; k <- 1L

  # Pick the smaller front element each time.
  while (i <= length(left) && j <= length(right)) {
    if (left[i] <= right[j]) {   # <= keeps sort stable
      result[k] <- left[i]; i <- i + 1L
    } else {
      result[k] <- right[j]; j <- j + 1L
    }
    k <- k + 1L
  }
  # Drain any remaining elements from either half.
  while (i <= length(left))  { result[k] <- left[i];  i <- i + 1L; k <- k + 1L }
  while (j <= length(right)) { result[k] <- right[j]; j <- j + 1L; k <- k + 1L }
  result
}

merge_sort <- function(v) {
  n <- length(v)
  if (n <= 1L) return(v)   # base case: zero or one element

  # Divide at the midpoint.
  mid   <- n %/% 2L
  left  <- merge_sort(v[1:mid])         # sort left half
  right <- merge_sort(v[(mid + 1L):n])  # sort right half

  merge_sorted(left, right)             # merge sorted halves
}
...
`,
    ...
    r: `# Lomuto partition: last element as pivot, returns pivot index
lomuto_partition <- function(v, lo, hi) {
  pivot <- v[hi]   # choose rightmost element as pivot
  i     <- lo - 1L # i tracks boundary of "less-than" region

  for (j in lo:(hi - 1L)) {
    if (v[j] <= pivot) {   # element belongs in left partition
      i    <- i + 1L
      tmp  <- v[i]; v[i] <- v[j]; v[j] <- tmp   # swap into left partition
    }
  }
  # Place pivot at its final sorted position.
  tmp       <- v[i + 1L]; v[i + 1L] <- v[hi]; v[hi] <- tmp
  list(v = v, p = i + 1L)
}

quick_sort <- function(v, lo = 1L, hi = length(v)) {
  if (lo >= hi) return(v)

  # Partition and get pivot index.
  res <- lomuto_partition(v, lo, hi)
  v   <- res$v
  p   <- res$p

  v <- quick_sort(v, lo,     p - 1L)   # recurse left of pivot
  v <- quick_sort(v, p + 1L, hi)       # recurse right of pivot
  v
}
...
func main() {
    ...
    r: `# Restore max-heap property at index root for heap of size n.
heapify <- function(v, n, root) {
  largest <- root          # assume root is largest
  left    <- 2L * root     # left child index  (1-indexed)
  right   <- 2L * root + 1L # right child index
  ...
  if (largest != root) {
    tmp         <- v[root]; v[root] <- v[largest]; v[largest] <- tmp
    v <- heapify(v, n, largest)   # fix the affected subtree
  }
  v
}
...
heap_sort <- function(v) {
  ...
  # Phase 1: build max-heap from last non-leaf downward.
  for (i in seq(n %/% 2L, 1L, by = -1L)) {
    v <- heapify(v, n, i)
  }

  # Phase 2: extract max element one at a time.
  for (i in seq(n, 2L, by = -1L)) {
    # Swap max (root) with last unsorted element.
    tmp  <- v[1L]; v[1L] <- v[i]; v[i] <- tmp
    # Restore heap property for the reduced heap.
    v <- heapify(v, i - 1L, 1L)
  }
  v
}
# Time: O(n log n)  Space: O(1)  Stable: FALSE
...
func main() {
    ...
    r: `shell_sort <- function(v) {
  n   <- length(v)
  gap <- n %/% 2L   # start with gap = n/2, halve each pass

  # Outer loop: keep going until gap reduces to zero.
  while (gap > 0L) {

    # Gapped insertion sort for this gap size.
    for (i in (gap + 1L):n) {
      temp <- v[i]   # element to be placed correctly
      j    <- i

      # Shift elements that are greater than temp by gap positions.
      while (j > gap && v[j - gap] > temp) {
        v[j] <- v[j - gap]   # move element gap positions ahead
        j    <- j - gap      # step back by one gap
      }
      v[j] <- temp   # place temp in its correct position
    }
    gap <- gap %/% 2L   # halve gap for next pass
  }
  v
}
# Time: O(n log² n)  Space: O(1)  Stable: FALSE
...
func main() {
    ...
    r: `counting_sort <- function(v) {
  if (length(v) == 0L) return(v)

  # Determine min and max to keep count vector compact.
  lo <- min(v); hi <- max(v)
  k  <- hi - lo + 1L   # range of values (k)

  # Count occurrences; offset index by lo to handle any integers.
  count <- integer(k)
  for (val in v) count[val - lo + 1L] <- count[val - lo + 1L] + 1L

  # Prefix-sum transform: count[i] = # elements <= i+lo-1.
  for (i in 2:k) count[i] <- count[i] + count[i - 1L]

  # Reconstruct sorted output right-to-left to preserve stability.
  output <- integer(length(v))
  for (i in seq(length(v), 1L, by = -1L)) {
    bucket        <- v[i] - lo + 1L      # offset index into count
    pos           <- count[bucket]       # correct output position
    output[pos]   <- v[i]
    count[bucket] <- count[bucket] - 1L  # decrement for next equal value
  }
  output
}
# Time: O(n+k)  Space: O(k)  Stable: TRUE

# ── demo ──
data <- c(4, 2, 2, 8, 3, 3, 1)
cat("Before:", data, "\n")
cat("After: ", counting_sort(data), "\n")
# Run: Rscript solution.R`,
...
func main() {
    ...
    r: `# One stable counting-sort pass keyed on digit at position exp (1, 10, 100, …)
counting_sort_digit <- function(v, exp) {
  n      <- length(v)
  output <- integer(n)
  count  <- integer(10L)   # one bucket per digit 0-9

  # Count frequency of each digit at this place value.
  for (val in v) {
    digit          <- (val %/% exp) %% 10L + 1L   # 1-indexed
    count[digit]   <- count[digit] + 1L
  }

  # Prefix-sum so count[d] is the end index for digit d.
  for (i in 2:10) count[i] <- count[i] + count[i - 1L]

  # Traverse right-to-left to preserve stability.
  for (i in seq(n, 1L, by = -1L)) {
    digit          <- (v[i] %/% exp) %% 10L + 1L
    count[digit]   <- count[digit] - 1L
    output[count[digit] + 1L] <- v[i]   # +1 because R is 1-indexed
  }
  output
}
...
radix_sort <- function(v) {
  ...
  max_val <- max(v)   # largest element determines digit count

  exp <- 1L
  # One pass per digit position while significant digits remain.
  while (max_val %/% exp > 0L) {
    v   <- counting_sort_digit(v, exp)   # stable sort by current digit
    exp <- exp * 10L
  }
  v
}
# Time: O(nk)  Space: O(n+k)  Stable: TRUE

# ── demo ──
data <- c(170, 45, 75, 90, 802, 24, 2, 66)
cat("Before:", data, "\n")
cat("After: ", radix_sort(data), "\n")
# Run: Rscript solution.R`,
...
func main() {
    ...
    r: `# Insertion sort a numeric vector in-place (stable, good for small inputs).
insertion_sort_bucket <- function(v) {
  n <- length(v)
  if (n <= 1L) return(v)
  for (i in 2:n) {
    key <- v[i]; j <- i - 1L
    while (j >= 1L && v[j] > key) { v[j + 1L] <- v[j]; j <- j - 1L }
    v[j + 1L] <- key
  }
  v
}

bucket_sort <- function(v) {
  n <- length(v)
  if (n <= 1L) return(v)

  # Determine value range to compute proportional bucket indices.
  lo   <- min(v); hi <- max(v)
  span <- if (hi == lo) 1 else hi - lo   # guard: all elements equal
  ...
  # Distribute each element into its corresponding bucket.
  for (val in v) {
    idx <- as.integer((val - lo) / span * n) + 1L
    if (idx > n) idx <- n   # clamp max element to last bucket
    buckets[[idx]] <- c(buckets[[idx]], val)
  }

  # Sort each bucket with insertion sort, then concatenate.
  result <- numeric(0)
  for (b in buckets) {
    if (length(b) > 0L) result <- c(result, insertion_sort_bucket(b))
  }
  result
}
# Time: O(n+k)  Space: O(n)  Stable: TRUE
...
func main() {
    ...
    r: `# Deque (double-ended queue) using an R environment.
# O(1) amortized push/pop at both ends via index bookkeeping.

new_deque <- function() {
  e        <- new.env(parent = emptyenv())
  e$items  <- list()

  e$push_back  <- function(x) e$items <- c(e$items, list(x))
  e$push_front <- function(x) e$items <- c(list(x), e$items)
  e$pop_front  <- function() {
    if (length(e$items) == 0L) stop("Deque is empty")
    val     <- e$items[[1L]]; e$items <- e$items[-1L]; val
  }
  e$pop_back   <- function() {
    n <- length(e$items)
    if (n == 0L) stop("Deque is empty")
    val     <- e$items[[n]]; e$items <- e$items[-n]; val
  }
  e$peek_front <- function() { if (length(e$items)==0L) stop("empty"); e$items[[1L]] }
  e$peek_back  <- function() { n <- length(e$items); if (n==0L) stop("empty"); e$items[[n]] }
  e$size       <- function() length(e$items)
  e$is_empty   <- function() length(e$items) == 0L
  e
}
...
cat("Front:", dq$peek_front(), "\n")   # 0
cat("Back: ", dq$peek_back(),  "\n")   # 2
cat("Pop front:", dq$pop_front(), "\n") # 0
cat("Pop back: ", dq$pop_back(),  "\n") # 2
cat("Size:", dq$size(), "\n")           # 1
# Run: Rscript solution.R`,
  },
...
func main() {
    ...
    r: `# Min-heap implemented as a 1-indexed integer vector.
...
new_heap <- function() {
  ...
  e$insert <- function(x) {
    e$data <- c(e$data, x)   # append to end
    i <- length(e$data)
    # Sift up: swap with parent while heap property violated.
    while (i > 1L && e$data[i %/% 2L] > e$data[i]) {
      tmp <- e$data[i %/% 2L]; e$data[i %/% 2L] <- e$data[i]; e$data[i] <- tmp
      i <- i %/% 2L
    }
  }
  e$extract_min <- function() {
    if (length(e$data) == 0L) stop("Heap is empty")
    mn      <- e$data[1L]
    n       <- length(e$data)
    e$data[1L] <- e$data[n]; e$data <- e$data[-n]
    # Sift down: swap with smallest child while heap property violated.
    i <- 1L; n <- length(e$data)
    repeat {
      l <- 2L*i; r <- 2L*i+1L; sm <- i
      if (l <= n && e$data[l] < e$data[sm]) sm <- l
      if (r <= n && e$data[r] < e$data[sm]) sm <- r
      if (sm == i) break
      tmp <- e$data[i]; e$data[i] <- e$data[sm]; e$data[sm] <- tmp
      i <- sm
    }
    mn
  }
  e$size <- function() length(e$data)
  e
}
...
func main() {
    ...
    r: `# Hash table using R's built-in named environment (open addressing via hashing).
# R environments are hash tables internally — this wraps one for clarity.

new_hash_table <- function() {
  e       <- new.env(hash = TRUE, parent = emptyenv())

  e$insert <- function(key, val) assign(key, val, envir = e$store)
  e$search <- function(key) {
    if (exists(key, envir = e$store, inherits = FALSE))
      get(key, envir = e$store, inherits = FALSE)
    else NULL
  }
  e$delete <- function(key) {
    if (exists(key, envir = e$store, inherits = FALSE))
      rm(list = key, envir = e$store)
  }
  e$store  <- new.env(hash = TRUE, parent = emptyenv())
  e
}
...
# ── demo ──
ht <- new_hash_table()
ht$insert("cat", 1L); ht$insert("dog", 2L); ht$insert("bat", 3L)
cat("search dog:", ht$search("dog"), "\n")   # 2
ht$delete("dog")
cat("after delete:", is.null(ht$search("dog")), "\n")   # TRUE
# Run: Rscript solution.R`,
  },
...
func main() {
    ...
    r: `# Binary Search Tree using recursive environments (nodes).
# Insert/Search: O(log n) avg   Space: O(n)

new_bst_node <- function(val) {
  e       <- new.env(parent = emptyenv())
  e$val   <- val; e$left <- NULL; e$right <- NULL; e
}

bst_insert <- function(node, val) {
  if (is.null(node)) return(new_bst_node(val))
  if (val < node$val)       node$left  <- bst_insert(node$left,  val)
  else if (val > node$val)  node$right <- bst_insert(node$right, val)
  node   # duplicate keys are ignored
}

bst_search <- function(node, val) {
  if (is.null(node))       return(FALSE)
  if (val == node$val)     return(TRUE)
  if (val < node$val)      return(bst_search(node$left,  val))
  return(bst_search(node$right, val))
}

bst_inorder <- function(node, acc = integer(0)) {
  if (is.null(node)) return(acc)
  acc <- bst_inorder(node$left, acc)
  acc <- c(acc, node$val)
  bst_inorder(node$right, acc)
}

# ── demo ──
root <- NULL
for (v in c(5, 3, 7, 1, 4)) root <- bst_insert(root, v)
cat("Inorder:", bst_inorder(root), "\n")   # 1 3 4 5 7
cat("Search 3:", bst_search(root, 3), "\n") # TRUE
cat("Search 6:", bst_search(root, 6), "\n") # FALSE
# Run: Rscript solution.R`,
  },
...
func main() {
    ...
    r: `# Graph (undirected) using an adjacency list stored in a named list.
# BFS/DFS: O(V+E)   Space: O(V)

new_graph <- function() {
  e     <- new.env(parent = emptyenv())
  e$adj <- list()   # named list: vertex -> integer vector of neighbors

  e$add_vertex <- function(v) {
    key <- as.character(v)
    if (is.null(e$adj[[key]])) e$adj[[key]] <- integer(0)
  }
  e$add_edge <- function(u, v) {
    e$add_vertex(u); e$add_vertex(v)
    ku <- as.character(u); kv <- as.character(v)
    e$adj[[ku]] <- c(e$adj[[ku]], v)   # edge u->v
    e$adj[[kv]] <- c(e$adj[[kv]], u)   # edge v->u (undirected)
  }
  e$bfs <- function(start) {
    visited <- character(0); order <- integer(0)
    queue   <- start
    visited <- c(visited, as.character(start))
    while (length(queue) > 0L) {
      v     <- queue[1L]; queue <- queue[-1L]   # dequeue front
      order <- c(order, v)
      for (nb in e$adj[[as.character(v)]]) {
        if (!as.character(nb) %in% visited) {
          visited <- c(visited, as.character(nb))
          queue   <- c(queue, nb)   # enqueue neighbor
        }
      }
    }
    order
  }
  e$dfs <- function(start) {
    visited <- character(0); order <- integer(0)
    dfs_rec <- function(v) {
      visited <<- c(visited, as.character(v)); order <<- c(order, v)
      for (nb in e$adj[[as.character(v)]])
        if (!as.character(nb) %in% visited) dfs_rec(nb)
    }
    dfs_rec(start); order
  }
  e
...
// Time: O(n log n)  Space: O(n)  Stable: YES`,
    ...
    r: `# Tim Sort: hybrid of insertion sort + merge sort
MIN_RUN <- 32L   # size of each small "run" to insertion-sort

# Insertion sort subarray v[left..right] in place (1-indexed).
insertion_sort_run <- function(v, left, right) {
  for (i in (left + 1L):right) {
    key <- v[i]   # element to place
    j   <- i - 1L
    # Shift elements right until correct position found.
    ...
    v[j + 1L] <- key   # insert element in correct spot
  }
  v
}

# Merge two sorted subarrays v[left..mid] and v[(mid+1)..right].
merge_run <- function(v, left, mid, right) {
  left_arr  <- v[left:mid]           # copy left half
  right_arr <- v[(mid + 1L):right]   # copy right half
  i <- 1L; j <- 1L; k <- left

  # Merge by picking the smaller front element each time.
  while (i <= length(left_arr) && j <= length(right_arr)) {
    if (left_arr[i] <= right_arr[j]) {   # <= keeps sort stable
      v[k] <- left_arr[i];  i <- i + 1L
    } else {
      v[k] <- right_arr[j]; j <- j + 1L
    }
    k <- k + 1L
  }
  # Drain any remaining elements from either half.
  while (i <= length(left_arr))  { v[k] <- left_arr[i];  i <- i + 1L; k <- k + 1L }
  while (j <= length(right_arr)) { v[k] <- right_arr[j]; j <- j + 1L; k <- k + 1L }
  v
}
...
tim_sort <- function(v) {
  ...
  # Step 1: sort individual runs of size MIN_RUN with insertion sort.
  i <- 1L
  while (i <= n) {
    right <- min(i + MIN_RUN - 1L, n)
    v <- insertion_sort_run(v, i, right)
    i <- i + MIN_RUN
  }
  ...
  while (size < n) {
    left <- 1L
    while (left <= n) {
      mid   <- min(left + size - 1L, n)
      right <- min(left + 2L * size - 1L, n)
      # Only merge if there is a right partition.
      if (mid < right) v <- merge_run(v, left, mid, right)
      left <- left + 2L * size
    }
    size <- size * 2L   # double the merge window
  }
  v
}
# Time: O(n log n)  Space: O(n)  Stable: TRUE
...
# Run: Rscript solution.R`,
  ...
  // ── ADAPTIVE SORT ──────────────────────────────────────────────────
  // Reads the data, then chooses cheapest path:
  //   counting sort (integer + small range) → insertion sort (tiny / nearly-sorted)
  //   → introsort with heapsort fallback (general)
  adaptive: {

    typescript: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
function ${fnName("adaptive", "typescript")}(input: number[]): number[] {
  const a = [...input];
  const n = a.length;
  if (n <= 1) return a;
  ...
  for (let i = 0; i + stride < n; i += stride) if (a[i] > a[i + stride]) inv++;
  const invRate = inv / Math.max(1, sample - 1);
  const span = maxVal - minVal + 1;

  // Path 1: counting sort for integers with small value range
  if (allInt && span < 4 * n) {
    const c = new Array(span).fill(0);
    for (let i = 0; i < n; i++) c[a[i] - minVal]++;
    let k = 0;
    for (let v = 0; v < span; v++) while (c[v]-- > 0) a[k++] = v + minVal;
    return a;
  }

  const ins = (lo: number, hi: number) => {
  ...
  // Path 2: tiny array or nearly-sorted → insertion sort
  if (n <= 16 || invRate <= 0.05) { ins(0, n - 1); return a; }

  // Path 3: introsort with heapsort fallback
  const maxDepth = 2 * Math.floor(Math.log2(n));
  const sift = (end: number, root: number, base: number): void => {
    let lg = root, l = 2 * root + 1, r = l + 1;
    if (l < end && a[base + l] > a[base + lg]) lg = l;
    if (r < end && a[base + r] > a[base + lg]) lg = r;
    if (lg !== root) { [a[base + root], a[base + lg]] = [a[base + lg], a[base + root]]; sift(end, lg, base); }
  };
  const heap = (lo: number, hi: number) => {
    const len = hi - lo + 1;
    for (let i = (len >> 1) - 1; i >= 0; i--) sift(len, i, lo);
    for (let i = len - 1; i > 0; i--) { [a[lo], a[lo + i]] = [a[lo + i], a[lo]]; sift(i, 0, lo); }
  };
  const med3 = (lo: number, mid: number, hi: number) => {
...
${statsLine("typescript", "O(n log n)", "O(log n)", false)}

${sectionDivider("typescript", "demo")}
const data = [64, 34, 25, 12, 22, 11, 90];
console.log("Before:", [...data]);
console.log("After: ", ${fnName("adaptive", "typescript")}(data));
${runLine("typescript")}`,

    javascript: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
function ${fnName("adaptive", "javascript")}(input) {
  const a = [...input];
  const n = a.length;
  if (n <= 1) return a;
  ...
  // Path 2: tiny / nearly sorted
  if (n <= 16 || invRate <= 0.05) { ins(0, n - 1); return a; }

  // Path 3: introsort
  const maxDepth = 2 * Math.floor(Math.log2(n));
  function sift(end, root, base) {
    let lg = root, l = 2 * root + 1, r = l + 1;
    if (l < end && a[base + l] > a[base + lg]) lg = l;
    if (r < end && a[base + r] > a[base + lg]) lg = r;
    if (lg !== root) { [a[base + root], a[base + lg]] = [a[base + lg], a[base + root]]; sift(end, lg, base); }
...
${statsLine("javascript", "O(n log n)", "O(log n)", false)}

${sectionDivider("javascript", "demo")}
const data = [64, 34, 25, 12, 22, 11, 90];
console.log("Before:", [...data]);
console.log("After: ", ${fnName("adaptive", "javascript")}(data));
${runLine("javascript")}`,

    python: `# Adaptive Sort — profiles input first, then picks the cheapest strategy.
import math

def ${fnName("adaptive", "python")}(arr):
    a = list(arr)
    n = len(a)
    if n <= 1:
        return a
    ...
    # Profile pass: min/max, integer check, sampled inversion rate
    min_val, max_val, all_int, inv = a[0], a[0], True, 0
    for x in a:
        if x < min_val: min_val = x
        if x > max_val: max_val = x
        if not isinstance(x, int): all_int = False
    sample = min(n, 40)
    stride = max(1, n // sample)
    i = 0
    while i + stride < n:
        if a[i] > a[i + stride]: inv += 1
        i += stride
    inv_rate = inv / max(1, sample - 1)
    span = max_val - min_val + 1

    # Path 1: counting sort
    if all_int and span < 4 * n:
        count = [0] * span
    ...
    def ins(lo, hi):
        for i in range(lo + 1, hi + 1):
            k = a[i]; j = i - 1
            while j >= lo and a[j] > k:
                a[j + 1] = a[j]; j -= 1
            a[j + 1] = k

    # Path 2: tiny / nearly sorted
    if n <= 16 or inv_rate <= 0.05:
        ins(0, n - 1)
        return a
    ...
    def sift(end, root, base):
        lg, l, r = root, 2 * root + 1, 2 * root + 2
        if l < end and a[base + l] > a[base + lg]: lg = l
        if r < end and a[base + r] > a[base + lg]: lg = r
        if lg != root:
            a[base + root], a[base + lg] = a[base + lg], a[base + root]
            sift(end, lg, base)

    def heap(lo, hi):
        length = hi - lo + 1
        for i in range(length // 2 - 1, -1, -1): sift(length, i, lo)
    ...
    def sort(lo, hi, depth):
        ...
        i, j = lo, hi - 2
        while True:
            while i <= hi - 1 and a[i] < pv: i += 1
            while j >= lo and a[j] > pv: j -= 1
            if i >= j: break
    ...
    sort(0, n - 1, max_depth)
    return a

${statsLine("python", "O(n log n)", "O(log n)", false)}

${sectionDivider("python", "demo")}
data = [64, 34, 25, 12, 22, 11, 90]
print("Before:", data)
print("After: ", ${fnName("adaptive", "python")}(data))
${runLine("python")}`,

    java: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
import java.util.Arrays;

public class Solution {
    static int[] a;

    static int[] ${fnName("adaptive", "java")}(int[] input) {
        a = input.clone();
        int n = a.length;
        if (n <= 1) return a;

        // Profile pass: min/max, sampled inversion rate
        int minVal = a[0], maxVal = a[0], inv = 0;
        for (int x : a) { if (x < minVal) minVal = x; if (x > maxVal) maxVal = x; }
        int sample = Math.min(n, 40);
        int stride = Math.max(1, n / sample);
        for (int i = 0; i + stride < n; i += stride) if (a[i] > a[i + stride]) inv++;
        double invRate = inv / (double) Math.max(1, sample - 1);
        long span = (long) maxVal - minVal + 1;

        // Path 1: counting sort for small range
        if (span < 4L * n) {
            int[] c = new int[(int) span];
            for (int x : a) c[x - minVal]++;
            int k = 0;
            for (int v = 0; v < span; v++) while (c[v]-- > 0) a[k++] = v + minVal;
            return a;
        }

        // Path 2: tiny or nearly sorted → insertion sort
        if (n <= 16 || invRate <= 0.05) { ins(0, n - 1); return a; }

        // Path 3: introsort with heapsort fallback
        sort(0, n - 1, 2 * (int) (Math.log(n) / Math.log(2)));
        return a;
    }

    static void ins(int lo, int hi) {
    ...
    static void sift(int end, int root, int base) {
        int lg = root, l = 2 * root + 1, r = l + 1;
        if (l < end && a[base + l] > a[base + lg]) lg = l;
        if (r < end && a[base + r] > a[base + lg]) lg = r;
        if (lg != root) { int t = a[base + root]; a[base + root] = a[base + lg]; a[base + lg] = t; sift(end, lg, base); }
    }

    static void heap(int lo, int hi) {
        int len = hi - lo + 1;
        for (int i = len / 2 - 1; i >= 0; i--) sift(len, i, lo);
        for (int i = len - 1; i > 0; i--) { int t = a[lo]; a[lo] = a[lo + i]; a[lo + i] = t; sift(i, 0, lo); }
    }

    static int med3(int lo, int mid, int hi) {
    ...
    static void sort(int lo, int hi, int depth) {
        if (hi - lo < 1) return;
        if (hi - lo + 1 <= 16) { ins(lo, hi); return; }
        if (depth == 0) { heap(lo, hi); return; }
        int mid = (lo + hi) >>> 1;
        int pi = med3(lo, mid, hi);
        int pv = a[pi];
        int t = a[pi]; a[pi] = a[hi - 1]; a[hi - 1] = t;
        int i = lo, j = hi - 2;
        while (true) {
            while (i <= hi - 1 && a[i] < pv) i++;
            while (j >= lo && a[j] > pv) j--;
            if (i >= j) break;
            int s = a[i]; a[i] = a[j]; a[j] = s; i++; j--;
        }
        int s = a[i]; a[i] = a[hi - 1]; a[hi - 1] = s;
        sort(lo, i - 1, depth - 1);
        sort(i + 1, hi, depth - 1);
    }
    ...
    public static void main(String[] args) {
        int[] data = {64, 34, 25, 12, 22, 11, 90};
        System.out.println("Before: " + Arrays.toString(data));
        System.out.println("After:  " + Arrays.toString(${fnName("adaptive", "java")}(data)));
    }
}
${statsLine("java", "O(n log n)", "O(log n)", false)}
${runLine("java")}`,

    c: `/* Adaptive Sort — profiles input first, then picks the cheapest strategy. */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void ${fnName("adaptive", "c")}(int *arr, int n) {
    a = arr;
    if (n <= 1) return;

    /* Profile pass */
    int minV = a[0], maxV = a[0], inv = 0;
    for (int i = 0; i < n; i++) { if (a[i] < minV) minV = a[i]; if (a[i] > maxV) maxV = a[i]; }
    int sample = n < 40 ? n : 40;
    int stride = n / sample > 1 ? n / sample : 1;
    for (int i = 0; i + stride < n; i += stride) if (a[i] > a[i + stride]) inv++;
    double invRate = (double) inv / (sample > 1 ? sample - 1 : 1);
    long span = (long) maxV - minV + 1;

    /* Path 1: counting sort */
    if (span < 4L * n) {
        int *c = (int *) calloc(span, sizeof(int));
        for (int i = 0; i < n; i++) c[a[i] - minV]++;
        int k = 0;
        for (int v = 0; v < span; v++) while (c[v]-- > 0) a[k++] = v + minV;
        free(c);
        return;
    }
    /* Path 2: tiny or nearly sorted */
    if (n <= 16 || invRate <= 0.05) { ins(0, n - 1); return; }
    /* Path 3: introsort */
    sort(0, n - 1, 2 * (int) log2((double) n));
}

int main(void) {
    int data[] = {64, 34, 25, 12, 22, 11, 90};
    int n = 7;
    ${fnName("adaptive", "c")}(data, n);
    for (int i = 0; i < n; i++) printf("%d ", data[i]);
    printf("\\n");
    return 0;
}
${statsLine("c", "O(n log n)", "O(log n)", false)}
${runLine("c")}`,

    cpp: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
#include <iostream>
#include <vector>
#include <algorithm>
#include <cmath>
...
static void sift(int end, int root, int base) {
    ...
    if (l < end && a[base + l] > a[base + lg]) lg = l;
    if (r < end && a[base + r] > a[base + lg]) lg = r;
    if (lg != root) { std::swap(a[base + root], a[base + lg]); sift(end, lg, base); }
}
static void heap(int lo, int hi) {
    int len = hi - lo + 1;
    for (int i = len / 2 - 1; i >= 0; i--) sift(len, i, lo);
    for (int i = len - 1; i > 0; i--) { std::swap(a[lo], a[lo + i]); sift(i, 0, lo); }
}
static int med3(int lo, int mid, int hi) {
...
std::vector<int> ${fnName("adaptive", "cpp")}(std::vector<int> input) {
    a = std::move(input);
    int n = a.size();
    if (n <= 1) return a;
    ...
    int sample = std::min(n, 40);
    int stride = std::max(1, n / sample);
    for (int i = 0; i + stride < n; i += stride) if (a[i] > a[i + stride]) inv++;
    double invRate = (double) inv / std::max(1, sample - 1);
    long span = (long) maxV - minV + 1;

    // Path 1: counting sort
    if (span < 4L * n) {
        std::vector<int> c(span, 0);
        for (int x : a) c[x - minV]++;
        int k = 0;
        for (int v = 0; v < span; v++) while (c[v]-- > 0) a[k++] = v + minV;
        return a;
    }
    ...
    if (n <= 16 || invRate <= 0.05) { ins(0, n - 1); return a; }
    // Path 3: introsort
    sort(0, n - 1, 2 * (int) std::log2(n));
    return a;
}

int main() {
    std::vector<int> data = {64, 34, 25, 12, 22, 11, 90};
    auto sorted = ${fnName("adaptive", "cpp")}(data);
    for (int x : sorted) std::cout << x << " ";
    std::cout << std::endl;
    return 0;
}
${statsLine("cpp", "O(n log n)", "O(log n)", false)}
${runLine("cpp")}`,

    rust: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
fn ins(a: &mut Vec<i32>, lo: usize, hi: usize) {
    for i in (lo + 1)..=hi {
        let k = a[i];
        let mut j = i;
        while j > lo && a[j - 1] > k { a[j] = a[j - 1]; j -= 1; }
        a[j] = k;
    }
}
fn sift(a: &mut Vec<i32>, end: usize, root: usize, base: usize) {
    let mut lg = root;
    let l = 2 * root + 1;
    let r = l + 1;
    if l < end && a[base + l] > a[base + lg] { lg = l; }
    if r < end && a[base + r] > a[base + lg] { lg = r; }
    if lg != root { a.swap(base + root, base + lg); sift(a, end, lg, base); }
}
fn heap(a: &mut Vec<i32>, lo: usize, hi: usize) {
    let len = hi - lo + 1;
    for i in (0..len / 2).rev() { sift(a, len, i, lo); }
    for i in (1..len).rev() { a.swap(lo, lo + i); sift(a, i, 0, lo); }
}
fn med3(a: &mut Vec<i32>, lo: usize, mid: usize, hi: usize) -> usize {
    if a[lo]  > a[mid] { a.swap(lo,  mid); }
    if a[lo]  > a[hi]  { a.swap(lo,  hi); }
    if a[mid] > a[hi]  { a.swap(mid, hi); }
    mid
}
fn sort(a: &mut Vec<i32>, lo: usize, hi: usize, depth: i32) {
    if hi <= lo { return; }
    if hi - lo + 1 <= 16 { ins(a, lo, hi); return; }
    if depth == 0 { heap(a, lo, hi); return; }
    let mid = (lo + hi) / 2;
    let pi = med3(a, lo, mid, hi);
    let pv = a[pi];
    a.swap(pi, hi - 1);
    let mut i = lo;
    let mut j = hi - 2;
    loop {
        while i <= hi - 1 && a[i] < pv { i += 1; }
        while j >= lo && a[j] > pv { if j == 0 { break; } j -= 1; }
        if i >= j { break; }
        a.swap(i, j); i += 1; j -= 1;
    }
    a.swap(i, hi - 1);
    if i > lo { sort(a, lo, i - 1, depth - 1); }
    sort(a, i + 1, hi, depth - 1);
}

fn ${fnName("adaptive", "rust")}(input: Vec<i32>) -> Vec<i32> {
    let mut a = input;
    let n = a.len();
    if n <= 1 { return a; }

    // Profile pass
    let (mut min_v, mut max_v, mut inv) = (a[0], a[0], 0);
    for &x in &a { if x < min_v { min_v = x; } if x > max_v { max_v = x; } }
    let sample = n.min(40);
    let stride = (n / sample).max(1);
    let mut i = 0;
    while i + stride < n { if a[i] > a[i + stride] { inv += 1; } i += stride; }
    let inv_rate = inv as f64 / (sample.max(2) - 1) as f64;
    let span = (max_v - min_v + 1) as i64;

    // Path 1: counting sort
    if span < 4 * n as i64 {
        let mut c = vec![0; span as usize];
        for &x in &a { c[(x - min_v) as usize] += 1; }
        let mut k = 0;
        for v in 0..span as usize {
            while c[v] > 0 { a[k] = v as i32 + min_v; k += 1; c[v] -= 1; }
        }
        return a;
    }
    // Path 2: tiny / nearly sorted
    if n <= 16 || inv_rate <= 0.05 { ins(&mut a, 0, n - 1); return a; }
    // Path 3: introsort
    let depth = 2 * (n as f64).log2() as i32;
    sort(&mut a, 0, n - 1, depth);
    a
}

fn main() {
    let data = vec![64, 34, 25, 12, 22, 11, 90];
    println!("{:?}", ${fnName("adaptive", "rust")}(data));
}
${statsLine("rust", "O(n log n)", "O(log n)", false)}
${runLine("rust")}`,

    go: `// Adaptive Sort — profiles input first, then picks the cheapest strategy.
package main
...
func heap(lo, hi int) {
    length := hi - lo + 1
    for i := length/2 - 1; i >= 0; i-- { sift(length, i, lo) }
    for i := length - 1; i > 0; i-- {
        a[lo], a[lo+i] = a[lo+i], a[lo]
        sift(i, 0, lo)
    }
}
func med3(lo, mid, hi int) int {
    if a[lo]  > a[mid] { a[lo],  a[mid] = a[mid], a[lo] }
    if a[lo]  > a[hi]  { a[lo],  a[hi]  = a[hi],  a[lo] }
    if a[mid] > a[hi]  { a[mid], a[hi]  = a[hi],  a[mid] }
    return mid
}
func sort(lo, hi, depth int) {
    if hi-lo < 1 { return }
    if hi-lo+1 <= 16 { ins(lo, hi); return }
    if depth == 0 { heap(lo, hi); return }
    mid := (lo + hi) / 2
...
func ${fnName("adaptive", "go")}(input []int) []int {
    a = append([]int(nil), input...)
    n := len(a)
    if n <= 1 { return a }

    // Profile pass
    minV, maxV, inv := a[0], a[0], 0
    for _, x := range a {
        if x < minV { minV = x }
        if x > maxV { maxV = x }
    }
    sample := n
    if 40 < sample { sample = 40 }
    stride := n / sample
    if stride < 1 { stride = 1 }
    for i := 0; i+stride < n; i += stride {
        if a[i] > a[i+stride] { inv++ }
    }
    sampleDenom := sample - 1
    if sampleDenom < 1 { sampleDenom = 1 }
    invRate := float64(inv) / float64(sampleDenom)
    span := int64(maxV - minV + 1)

    // Path 1: counting sort
    if span < 4*int64(n) {
        c := make([]int, span)
        for _, x := range a { c[x-minV]++ }
        k := 0
        for v := int64(0); v < span; v++ {
            for c[v] > 0 { a[k] = int(v) + minV; k++; c[v]-- }
        }
        return a
    }
    // Path 2: tiny / nearly sorted
    if n <= 16 || invRate <= 0.05 { ins(0, n-1); return a }
    // Path 3: introsort
    sort(0, n-1, 2*int(math.Log2(float64(n))))
    return a
}

func main() {
    data := []int{64, 34, 25, 12, 22, 11, 90}
    fmt.Println(${fnName("adaptive", "go")}(data))
}
${statsLine("go", "O(n log n)", "O(log n)", false)}
${runLine("go")}`,

    r: `# Adaptive Sort — profiles input first, then picks the cheapest strategy.
${fnName("adaptive", "r")} <- function(input) {
  a <- input
  n <- length(a)
  if (n <= 1) return(a)
  ...
  all_int <- all(a == as.integer(a))
  sample <- min(n, 40L)
  stride <- max(1L, n %/% sample)
  inv <- 0L
  i <- 1L
  while (i + stride <= n) {
    if (a[i] > a[i + stride]) inv <- inv + 1L
    i <- i + stride
  }
  inv_rate <- inv / max(1L, sample - 1L)
  span <- max_v - min_v + 1L

  # Path 1: counting sort
  if (all_int && span < 4 * n) {
    cnt <- integer(span)
    for (x in a) cnt[x - min_v + 1L] <- cnt[x - min_v + 1L] + 1L
    k <- 1L
    for (v in seq_len(span)) {
      while (cnt[v] > 0L) {
        a[k] <- v - 1L + min_v
        k <- k + 1L
        cnt[v] <- cnt[v] - 1L
  ...
  ins <- function(lo, hi) {
    for (i in (lo + 1L):hi) {
      k <- a[i]; j <- i - 1L
      while (j >= lo && a[j] > k) { a[j + 1L] <<- a[j]; j <- j - 1L }
      a[j + 1L] <<- k
    }
  }

  # Path 2: tiny / nearly sorted
  if (n <= 16L || inv_rate <= 0.05) { ins(1L, n); return(a) }

  # Path 3: introsort
  max_depth <- 2L * floor(log2(n))
  sift <- function(end, root, base) {
    lg <- root; l <- 2L * root + 1L; r <- l + 1L
    if (l < end && a[base + l] > a[base + lg]) lg <- l
    if (r < end && a[base + r] > a[base + lg]) lg <- r
    if (lg != root) {
      tmp <- a[base + root]; a[base + root] <<- a[base + lg]; a[base + lg] <<- tmp
      sift(end, lg, base)
    }
  }
  heap <- function(lo, hi) {
    len <- hi - lo + 1L
    for (k in (len %/% 2L - 1L):0L) sift(len, k, lo - 1L)
    for (k in (len - 1L):1L) {
      tmp <- a[lo]; a[lo] <<- a[lo + k]; a[lo + k] <<- tmp
      sift(k, 0L, lo - 1L)
    }
  }
  med3 <- function(lo, mid, hi) {
    if (a[lo]  > a[mid]) { tmp <- a[lo]; a[lo] <<- a[mid]; a[mid] <<- tmp }
    if (a[lo]  > a[hi])  { tmp <- a[lo]; a[lo] <<- a[hi];  a[hi]  <<- tmp }
    if (a[mid] > a[hi])  { tmp <- a[mid]; a[mid] <<- a[hi]; a[hi] <<- tmp }
    mid
  }
  sort_rec <- function(lo, hi, depth) {
    if (hi - lo < 1L) return(invisible())
    if (hi - lo + 1L <= 16L) { ins(lo, hi); return(invisible()) }
    if (depth == 0L) { heap(lo, hi); return(invisible()) }
    mid <- (lo + hi) %/% 2L
    pi <- med3(lo, mid, hi); pv <- a[pi]
    tmp <- a[pi]; a[pi] <<- a[hi - 1L]; a[hi - 1L] <<- tmp
    i <- lo; j <- hi - 2L
    repeat {
      while (i <= hi - 1L && a[i] < pv) i <- i + 1L
      while (j >= lo && a[j] > pv) j <- j - 1L
      if (i >= j) break
      tmp <- a[i]; a[i] <<- a[j]; a[j] <<- tmp; i <- i + 1L; j <- j - 1L
    }
    tmp <- a[i]; a[i] <<- a[hi - 1L]; a[hi - 1L] <<- tmp
    sort_rec(lo, i - 1L, depth - 1L)
    sort_rec(i + 1L, hi, depth - 1L)
  }
  sort_rec(1L, n, max_depth)
  a
}
${statsLine("r", "O(n log n)", "O(log n)", false)}

${sectionDivider("r", "demo")}
data <- c(64, 34, 25, 12, 22, 11, 90)
cat("Before:", data, "\n")
cat("After: ", ${fnName("adaptive", "r")}(data), "\n")
${runLine("r")}`,
...
def logos_sort(arr):
    ...
    return a
# Time: O(n log n)  Space: O(log n)  Stable: False

# ── demo ──
data = [64, 34, 25, 12, 22, 11, 90, 42, 5, 77]
print("Before:", data[:])
print("After: ", logos_sort(data))
# Run: python solution.py`,
...
pub fn logos_sort(arr: &mut Vec<i32>) {
    let n = arr.len();
    if n < 2 { return; }
...
// Time: O(n log n)  Space: O(log n)  Stable: NO`,
    ...
    r: `# Logos Sort — λόγος: the reason that orders the cosmos.
#
# Nine gifts, given only when the moment calls:
#   (1) tally before you touch    — count, do not compare, when the range permits
#   (2) read before you act       — already in order? already reversed? then rest
...
logos_sort <- function(arr) {
  n <- length(arr)
  if (n < 2L) return(arr)   # one element needs no ordering
  a <- arr                   # R is pass-by-value — work on a copy

  depth_limit <- as.integer(2L * floor(log2(n))) + 4L  # patience, not infinite

  # Three candidates enter. The middle one alone carries the truth.
  median3 <- function(x, y, z) {
    if (x > y) { t <- x; x <- y; y <- t }
    if (y > z) { t <- y; y <- z; z <- t }
    if (x > y) { t <- x; x <- y; y <- t }
    y
  ...
  # Consult the neighbors — the outlier is overruled; the center of three is truth.
  ninther <- function(lo, hi, idx) {
    i0 <- max(lo, idx - 1L); i2 <- min(hi, idx + 1L)
    median3(a[i0], a[idx], a[i2])
  }

  # Two thresholds cleave the world into three.
  dual_partition <- function(lo, hi, p1, p2) {
    if (p1 > p2) { t <- p1; p1 <- p2; p2 <- t }
    lt <- lo; gt <- hi; i <- lo
    while (i <= gt) {
      if (a[i] < p1) {
        # Element belongs in left region — swap with left boundary.
        t <- a[lt]; a[lt] <<- a[i]; a[i] <<- t
        lt <- lt + 1L; i <- i + 1L
      } else if (a[i] > p2) {
        # Element belongs in right region — swap with right boundary.
        t <- a[i]; a[i] <<- a[gt]; a[gt] <<- t
        gt <- gt - 1L
      } else {
        i <- i + 1L   # element is in [p1, p2]: already in place
      }
    }
    c(lt, gt)
  }

  # The great descent — tail-recurse the largest piece for free.
  sort_rec <- function(lo, hi, depth) {
    while (lo < hi) {
      size <- hi - lo + 1L

      # Wisdom knows its limits — yield to base sort when depth is exhausted.
      if (depth <= 0L) {
        a[lo:hi] <<- sort(a[lo:hi])
      ...
      if (size <= 48L) {
        for (i in (lo + 1L):hi) {
          key <- a[i]; j <- i - 1L
          while (j >= lo && a[j] > key) { a[j + 1L] <<- a[j]; j <- j - 1L }
          a[j + 1L] <<- key
        }
        return(invisible(NULL))
      }
      ...
      mn <- min(a[lo:hi]); mx <- max(a[lo:hi])
      span <- mx - mn
      if (is.integer(a) && span < size * 4L) {
        counts <- integer(span + 1L)
        for (k in lo:hi) counts[a[k] - mn + 1L] <- counts[a[k] - mn + 1L] + 1L
        k <- lo
        for (v in seq_len(span + 1L)) {
          while (counts[v] > 0L) { a[k] <<- v + mn - 1L; k <- k + 1L; counts[v] <- counts[v] - 1L }
        }
        return(invisible(NULL))
      }
      ...
      if (a[lo] <= a[lo + 1L] && a[lo + 1L] <= a[lo + 2L]) {
        sorted <- all(diff(a[lo:hi]) >= 0)
        if (sorted) return(invisible(NULL))
        reversed <- all(diff(a[lo:hi]) <= 0)
        if (reversed) {
          a[lo:hi] <<- rev(a[lo:hi])
          return(invisible(NULL))
        }
  ...
  sort_rec(1L, n, depth_limit)
  a
}
# Time: O(n log n)  Space: O(log n)  Stable: FALSE

# ── demo ──
data <- c(64, 34, 25, 12, 22, 11, 90, 42, 5, 77)
cat("Before:", data, "\n")
cat("After: ", logos_sort(data), "\n")
# Run: Rscript solution.R`,
